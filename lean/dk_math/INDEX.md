...
- `def Progress` / `def HasCycle` / `def Mixable`
- `theorem not_mixable_of_progress`
...
（`__theorems-heading.txt` と `__sorries.txt` から抽出した“現時点の集計”。）

- 定義/補題/定理総数（`def/lemma/theorem` 抽出）：**819**
- `sorry` 総数（コード上の `sorry` 行を抽出）：**26**

### 未完集中（`sorry` 抽出）

- `CosmicFormula/TriominoFLT.lean`（5）
- `NumberTheory/GcdNext.lean`（6）
- `FLT/Basic.lean`（6）
- `NumberTheory/ZsigmondyCyclotomic.lean`（2）
- `UniqueRepresentation.lean`（1）
- `CellDim.lean`（1）
- `NumberTheory/GcdLemmas.lean`（1）
- `FLT/Samples.lean`（2）
- `Collatz/V2#pending.lean`（2）

→ 依存構造的には **GcdNext と FLT.Basic が“橋の未完”**、 Zsigmondy 本体の `sorry` は少数（ただし重い箇所）という形。
