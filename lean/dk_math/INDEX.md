...
- \*\*コード行先頭の `sorry`\*\*として抽出（このスナップショット）：**19 件**

#### `sorry` 集中（上位）

- `DkMath/CosmicFormula/TriominoFLT.lean`（6）
- `DkMath/NumberTheory/GcdNext.lean`（5）
- `DkMath/NumberTheory/ZsigmondyCyclotomic.lean`（2）
- `DkMath/Collatz/V2#pending.lean`（2）
- `DkMath/FLT/Basic.lean`（1）
- `DkMath/NumberTheory/GcdLemmas.lean`（1）
- `DkMath/UniqueRepresentation.lean`（1）
- `DkMath/CellDim.lean`（1）

→ 構造的には **GcdNext と TriominoFLT が“未完の塊”**。 Zsigmondy 本体は 2 件だが、ここは性質上「重い」ので要注意。
