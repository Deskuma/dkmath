...
def test_theorem_picker_short_option():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # 少なくとも1つの定義が必要
        assert (
            simple_def_present or another_def_present
        ...
        simple_theorem_match = re.search(
            r"theorem simple_theorem.*?```", content, re.DOTALL
        )
        assert (
            simple_theorem_match is not None
        ...
        assert (
            "by ..." in simple_theorem_text
        ), "Expected `simple_theorem` to contain `by ...` with --short option."
        assert (
            "trivial" not in simple_theorem_text
