...
/-! Astra-002: a second, source-sensitive equation excludes the calibration unit.
This scratch preserves the current source packet; it is not a Thue completeness proof.
-/

namespace DkMathTest.FLT.SevenCalibrationExclusionAstra02Scratch
...
theorem source_root_seventh_plane
...
def calibrationQuintic (a K : ℤ) : ℤ :=
...
theorem calibration_line_impossible (a K : ℤ)
    (hK : K ≠ 0) (hK7 : (7 : ℤ) ∣ K)
  ...
  have hpoly : calibrationQuintic a K = 0 :=
    (mul_eq_zero.mp hprod).resolve_left (mul_ne_zero (by norm_num) (pow_ne_zero _ hK))
...
theorem calibration_gap_coordinates (v : SevenRealCubicInt) (K : ℤ)
...
end DkMathTest.FLT.SevenCalibrationExclusionAstra02Scratch
