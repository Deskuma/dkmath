...
で見ている全ての base value $\mathrm{pOf}(q)$ が $1$ 以上である、という条件じゃ。
...
$$
totalWeight = \sum_{q\in I}\frac{\log(\mathrm{pOf}(q))}{\log n}
$$
...
へ進む道。
ただし、ここでは $\mathrm{pOf}(q)$ の重複制御、積の正性、log の積公式、単調性が必要になる。
...
RealLogBudget
\Rightarrow
\sum \log(\mathrm{pOf}(q))/\log n\le 1
$$
