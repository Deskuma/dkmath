...
### 4.1.3. さらに確定したこと（2026/04/08 05:41 JST 時点）

- `primeOrY_of_chosenFactorInP_and_tailSumInP_of_counterexamplePack` を追加し、
  chosen factor と local tail-sum が同じ prime ideal に入るなら
  `P ∣ (p) ∨ y ∈ P` が product-free に出ることを fixed した。
- それを用いて
  `chosenLinearFactor_isCoprime_with_tailSum_of_firstCase_of_pack_withoutProduct`
  を追加し、tail-sum ideal の coprimality bridge も no-sorry で閉じた。
- その結果、local tailsum route だけから
  ...
  が立った。
- さらにこの product-free normalization を使って、
  `CyclotomicNormGNPowerFirstCasePackThinTarget` とその contradiction wrapper からも
  `hProduct` を外せた。
- よって current first-case mainline では、Stage 1 / Stage 2 / Stage 3 の concrete chain は
  **本質的に hProduct-free** と見てよい段階へ進んだ。
...
2026/04/08 05:41 JST 更新:

- tail-sum ideal の coprimality bridge も立ったため、
  first-case concrete chain に限ればここは実質「大勝利」へ到達したと見てよい。
