...
### 2026-04-17: class-type reducibility warning cleanup

`Definition ... of class type must be marked with @[reducible] or @[implicit_reducible]` の warning を整理した。
今回は warning ごとに意味を分けて確認し、いずれも `@[implicit_reducible]` が妥当と判断した。

#### 切り分け

- `DkMath/DHNT/UnitNatLayers.lean`
  - `Bridge` は class だが、`piBridge` などは「具体的な bridge 値」を明示引数として渡す用途。
  - 普通の abbreviation として常時展開する必要はなく、class 値として必要時に unfold できれば十分なので `@[implicit_reducible]` を採用した。

- `DkMath/FLT/GEisensteinBridge.lean`
  - `numberTheoryHasKernelFamily_of_*` は「class/instance family を返す関数」。
  - 意味的には instance adapter に近く、型クラス探索や implicit 引数の整合時に unfold できれば足りるため `@[implicit_reducible]` を採用した。

- `DkMath/FLT/Kummer/CyclotomicPrincipalization.lean`
  - `IsCyclotomicExtension_p_as_pow1` は単発の class adapter。
  - `{p}` から `{p^(0+1)}` へ class 値を持ち上げるだけなので、これも `@[implicit_reducible]` を採用した。
...
- `DkMath/DHNT/UnitNatLayers.lean`
  ...
  - `floorBridge`
  - `scale10Bridge`
  - `rationalApproxBridge`
  - `constructibleBridge`
  - 以上に `@[implicit_reducible]` を付与した。

- `DkMath/FLT/GEisensteinBridge.lean`
  - `numberTheoryHasKernelFamily_of_hasStep`
  - `numberTheoryHasKernelFamily_of_hasReduce`
  - `numberTheoryHasKernelFamily_of_hasStepExists`
  - `numberTheoryHasKernelFamily_of_hasLocalReduce`
  - `numberTheoryHasKernelFamily_of_nonLiftableFamily`
  - `numberTheoryHasKernelFamily_of_hasNoSqFamily`
  - 以上に `@[implicit_reducible]` を付与した。

- `DkMath/FLT/Kummer/CyclotomicPrincipalization.lean`
  - `IsCyclotomicExtension_p_as_pow1`
  - に `@[implicit_reducible]` を付与した。

#### 確認

- 個別確認
  - `lake build DkMath.DHNT.UnitNatLayers`
  - `lake build DkMath.FLT.GEisensteinBridge`
  - `lake build DkMath.FLT.Kummer.CyclotomicPrincipalization`

- 全体確認
  - `lake build > ./tmp/v429-reducible-check.log 2>&1`
  - `rg "class type must be marked with" ./tmp/v429-reducible-check.log`
  - full build ログ上で該当 warning が 0 件であることを確認した。
