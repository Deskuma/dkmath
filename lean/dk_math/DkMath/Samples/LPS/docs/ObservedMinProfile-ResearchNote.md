...
- `Samples/LPS` は LPS 本体の、Big-family 語彙で現象を観測する実験層として機能している。
...
この接続は、現段階では標本整理のための DHNT 作業仮説である。
...
### 11.11. 増殖段階タイムライン（`Big`, One/Two, 新規 `ΔBody`）

`profile_map_summary.csv` と `11.10` の初出表を統合すると、
立方世界の成長段階は次の時系列で読める。

| Big (=m^3) | One-candidate | ΔOne | Two-candidate | ΔTwo | 新規 `ΔBody` | 備考 |
|---:|---:|---:|---:|---:|---|---|
| 27  | 4  | -  | 3  | -  | {1, 6, 11} | 初期核の立ち上がり |
| 64  | 5  | +1 | 6  | +3 | {10} | Two 島が One を上回る開始点 |
| 125 | 6  | +1 | 9  | +3 | {34} | 長距離切替間隔が追加 |
| 216 | 7  | +1 | 14 | +5 | {27} | One/Two 分岐の再現が明瞭化 |
| 343 | 8  | +1 | 20 | +6 | {2} | 近接切替の新型が出現 |
| 512 | 9  | +1 | 26 | +6 | {44} | 最大級の切替間隔が出現 |
| 729 | 10 | +1 | 34 | +8 | ∅ | `ΔBody` 候補集合はこの時点で飽和 |

この表は、

- profile 島の個数増加（One/Two）
- 境界間隔の新規出現（`ΔBody`）
...
- `ΔOne` はこの範囲で一貫して `+1`。
- `ΔTwo` は `+3, +3, +5, +6, +6, +8` と段階的に増加し、
   Two 側の増殖速度が One 側より高いことを示す。

### 11.12. `m` と `ΔTwo` の対応（簡易表）

`Big = m^3` に対する `ΔTwo`（前段差分）を抜き出すと次になる。

| m | Big (=m^3) | ΔTwo | 同段での新規 `ΔBody` |
|---:|---:|---:|---|
| 4 | 64  | +3 | {10} |
| 5 | 125 | +3 | {34} |
| 6 | 216 | +5 | {27} |
...
- `ΔTwo` は `m` の増加とともに単調ではないが全体として増勢を示す。
- 大きい新規 `ΔBody` が出る段（例: `34`, `44`）と、`ΔTwo` 増加の段が
   近接して現れる傾向がある。

## 12. 粗視化↔微視化交換則（試算転記）

`python/LPS/DHNT/experiment_exchange_balance.py` の出力を
研究ノート用に転記する。

### 12.1. 粗視化↔微視化交換例（`A^m = a^n`）

出典: `python/LPS/DHNT/docs/exchange_coarse_fine_examples.csv`

| value | 粗い表示 | 細かい表示 | 分割比 `t` | valid |
|---:|---|---|---:|---:|
| 16   | `4^2`   | `2^4`   | 2 | 1 |
| 81   | `9^2`   | `3^4`   | 2 | 1 |
| 64   | `8^2`   | `2^6`   | 3 | 1 |
...
- ここで `valid=1` は、粗い表示と細かい表示が同値を与えることを示す。
- 分割比 `t` は `A = a^t` の指数であり、次数倍率 `n = t m` に一致する。

### 12.2. 多段粒度鎖（同一値の多重表示）

出典: `python/LPS/DHNT/docs/exchange_multiresolution_chain.csv`

| value | expr | eval | valid |
|---:|---|---:|---:|
| 4096 | `64^2`  | 4096 | 1 |
| 4096 | `8^4`   | 4096 | 1 |
| 4096 | `4^6`   | 4096 | 1 |
...
- 同一値が複数粒度で連鎖表示できることを確認。
- 粗視化→微視化に伴って次数が段階的に増加する。

### 12.3. 反転平衡の連続族（`a^b = b^a`）

出典: `python/LPS/DHNT/docs/powerswap_continuous_samples.csv`

| t | a | b | a^b | b^a | relative_error | balanced |
|---:|---:|---:|---:|---:|---:|---:|
| 0.5 | 4.0 | 2.0 | 16.0 | 16.0 | 0.0 | 1 |
| 2.0 | 2.0 | 4.0 | 16.0 | 16.0 | 0.0 | 1 |
| 3.0 | 1.7320508075688772 | 5.196152422706632 | 17.361905250953132 | 17.361905250953132 | 0.0 | 1 |
| 4.0 | 1.5874010519681994 | 6.3496042078727974 | 18.805326789549465 | 18.805326789549472 | 3.7784120622405996e-16 | 1 |

観測メモ:

- 整数格子点 `(2,4)` は連続族の一標本として再現される。
- `relative_error` は浮動小数誤差で、`balanced=1` は平衡成立を示す。

### 12.4. 交換条件の最小形（命題）

粗視化↔微視化交換の最小条件を、次の命題形で固定する。

> **命題（交換条件の最小形）**
> `A = a^t`（`a, A, t ∈ ℕ`, `a ≥ 2`, `t ≥ 1`）なら、任意の `m ∈ ℕ` で
> `A^m = a^(t m)` が成り立つ。
...
この命題は、`12.1` の試算表に現れる
「分割比 `t` が次数倍率 `n = t m` を与える」規則の代数的核となる。
...
- 実装:
  ...
  - `powerSwap_branch_at_three_y_eq_three_mul_x`
  - `powerSwap_branch_correct_at_two`
...
- 実装:
  ...
  - 局所モデル: `localX`, `localY`, `gapQuadraticModel`
  - 最小補題: `gapQuadraticModel_swap_neg`, `gapQuadraticModel_diag_zero`,
