...
/-- An FLT-style equation forces one side to be generated by the Power Gap/Beam
    factorization against the other side and the top term. -/
theorem flt_eq_forces_powerGapBeam {R : Type*} [CommRing R]
    (d : ℕ) (x y z : R)
    (h : x ^ d + y ^ d = z ^ d) :
...
/-- Symmetric form of `flt_eq_forces_powerGapBeam`, observing the other side
    as a Power Gap/Beam product. -/
theorem flt_eq_forces_powerGapBeam_symm {R : Type*} [CommRing R]
    (d : ℕ) (x y z : R)
    (h : x ^ d + y ^ d = z ^ d) :
