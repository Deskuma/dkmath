...
証明のアイデア（高レベル）:
  ...
  3. (1) の和を k=0 の項（即ち u^d）と k≥1 の和に分離する。
     ...
     (x+u)^d から x * GReal d x u を引くと残るのは u^d だけになる。
...
まとめ:
  二項展開の k=0 項が目標の u^d を与え、残る項は x*GReal の展開と対応して互いに打ち消すため、等式が成立する。
-/
theorem cosmic_id (d : ℕ) (x u : ℝ) :
    (x + u)^d - x * GReal d x u = u^d := by
  unfold GReal
  rw [add_pow, Finset.mul_sum]
  -- 二項定理: (x+u)^d = Σ_{k=0}^{d} C(d,k) x^k u^{d-k}
  -- GReal の展開: x * GReal = Σ_{j=0}^{d-1} C(d,j+1) x^{j+1} u^{d-1-j}
  ...
  have h1 : ∑ k ∈ Finset.range (d + 1), x ^ k * u ^ (d - k) * ↑(d.choose k)
    ...
    rw [h2, h3]
  -- 補題2: x * GReal を展開すると、補題1の第2項と同じ形になる
