...
> つまりある多項式 $G(x,u) \in \mathbb{R}[x,u]$ が存在して $(x+u)^d - u^d = x \cdot G(x,u)$ となる。
> ここでの「 $/x$ 」は多項式除法（商多項式を取ること）であり $x$ の逆元を要しない。
> 留意点：ある $r\in\mathbb{R}$ を代入してから両辺を $\mathbb{R}$ 内で割りたい（数として $/r$ をとる）なら、その代入値 $r$ が $\mathbb{R}$ の単元である必要がある。また、等式 $x\cdot A = x\cdot B$ から $A = B$ を結論づけるには一般に $x$ が零因子でないこと（あるいは可逆であること）が必要となる。
