...
#print "file: DkMath.ABC.ChernoffDensity"
...
noncomputable instance decidable_Bad_ε (γ_values : ℕ → ℝ) :
    DecidablePred fun n => Bad_ε n γ_values :=
...
lemma bad_set_density_bound_param
  ...
  (Λ : ℝ) (hΛ_nonneg : 0 ≤ Λ)
  (HΛ : ∀ X, X ≥ const_X →
    ∑ p ∈ primesUpTo X, (p : ℝ) ^ (-(Real.log 2 / Real.log 3) * (γ_values p + 2)) ≤ Λ) :
  ∃ C > 0, ∀ X ≥ const_X,
  ...
  have h_sub :
    (Finset.filter (fun n => Bad_ε n γ_values) (Finset.Icc 0 X))
      ⊆ Finset.biUnion (primesUpTo X) fun p =>
    ...
    have hv_ge1 : 1 ≤ padicValNat p (2 * n + 1) := by
      linarith [Nat.succ_le_iff.mpr hv_pos]
  ...
  have h_card_le :
    ...
    calc
      ((Finset.filter (fun n => Bad_ε n γ_values) (Finset.Icc 0 X)).card : ℝ)
          ≤ (Finset.biUnion (primesUpTo X) fun p =>
                Finset.filter (fun n => n ≤ X ∧ Excess p (γ_values p) n) (Finset.Icc 0 X)).card := by
  ...
  calc
    ((Finset.filter (fun n => Bad_ε n γ_values) (Finset.Icc 0 X)).card : ℝ)
        ≤ ∑ p ∈ primesUpTo X,
            ((Finset.filter (fun n => n ≤ X ∧ Excess p (γ_values p) n) (Finset.Icc 0 X)).card : ℝ) := h_card_le
    ...
    _ ≤ const_C * (X : ℝ) * Λ := by
          ...
          · have hCpos_local : 0 < (const_C : ℝ) := by norm_num [const_C]
            ...
            exact mul_nonneg (le_of_lt hCpos_local) hX_nonneg
    _ ≤ (const_C * (Λ + 1)) * (X : ℝ) := by
          have hΛ_add : Λ ≤ Λ + 1 := by linarith [hΛ_nonneg]
          have : (const_C : ℝ) * (Λ + 1) * (X : ℝ) = (const_C : ℝ) * (X : ℝ) * (Λ + 1) := by
            ring
          rw [this]
...
lemma bad_set_density_bound'
  ...
  have HΛ : ∀ X, X ≥ const_X →
      ∑ p ∈ primesUpTo X, (p : ℝ) ^ (-(Real.log 2 / Real.log 3) * (γ_values p + 2)) ≤ Λ := by
    intro X hX
    have hN := hseries X
    have h_pow_le : ∀ p ∈ primesUpTo X,
      ...
      have hpos : 0 < Real.log 2 / (2 * Real.log 3) := by
        exact div_pos hlog2_pos (mul_pos (by norm_num : (2 : ℝ) > 0) hlog3_pos)
      have diff_eq :
          (-(Real.log 2 / (2 * Real.log 3)) * s) - (-(Real.log 2 / Real.log 3) * s)
            = (Real.log 2 / (2 * Real.log 3)) * s := by
        ring_nf
      have pos_nonneg : 0 ≤ (Real.log 2 / (2 * Real.log 3)) * s := by
        apply mul_nonneg (le_of_lt hpos) (le_of_lt s_pos)
      have exp_le :
          (-(Real.log 2 / Real.log 3) * s) ≤ (-(Real.log 2 / (2 * Real.log 3)) * s) := by
