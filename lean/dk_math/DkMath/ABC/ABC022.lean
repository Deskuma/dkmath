...
- mgf_padic_excess_bound_pbase_layercake:
  `exp / rpow` の layer‑cake，`count_powers_dividing_2n1`，幾何級数評価を結合して，
...
lemma rpow_layer_cake
  ...
  have h_lhs : Finset.sum (Finset.Icc 0 X) (fun n => a ^ (t * (V n : ℝ)))
    ...
    ring
  -- Now we have the `exp_layer_cake` bound for the LHS,
  -- we need to convert the RHS back to `a^x` form
  rw [h_lhs]
  -- Convert RHS constants
  have h_at : a ^ t = Real.exp t' := by
    rw [h_rpow_exp, show t' = t * Real.log a from rfl]
  -- Convert the k-th term in the sum: `a^{t(k-1)} = exp(t'*(k-1))`
...
lemma exp_layer_cake'
  ...
  have h_decomp : ∀ n ≤ X, Real.exp (t * (V n : ℝ))
    ...
    · simp [hV]
    · -- Telescoping sum
      have h_tele : Finset.sum (Finset.Icc 1 (V n))
