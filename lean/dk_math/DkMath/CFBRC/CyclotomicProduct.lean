...
@[simp] def cyclotomicShiftedEval {R : Type _} [CommRing R]
    (m : ℕ) (x u : R) : R :=
  ∑ k ∈ Finset.range ((Polynomial.cyclotomic m ℤ).natDegree + 1),
    ((Polynomial.cyclotomic m ℤ).coeff k : R) * (x + u) ^ k *
      u ^ ((Polynomial.cyclotomic m ℤ).natDegree - k)
...
theorem cyclotomicShiftedEval_one_eq_cyclotomicEval_add_one
    {R : Type _} [CommRing R] (m : ℕ) (x : R) :
    cyclotomicShiftedEval m x 1 = cyclotomicEval m (x + 1) := by
  unfold cyclotomicShiftedEval cyclotomicEval
...
theorem cyclotomicDivisorsProductShifted_one_eq_geomSum
    {R : Type _} [CommRing R] {d : ℕ} (hd : 0 < d) (x : R) :
