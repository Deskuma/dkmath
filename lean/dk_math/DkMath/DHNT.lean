...
/-- 正の実数を単位と見なす
※ Unit は予約語なので DUnit (Dimensioned Unit)
-/
@[ext] structure DUnit where
  val : ℝ
  pos : 0 < val

@[simp] instance : Mul DUnit where
  mul := fun u v => ⟨u.val * v.val, mul_pos u.pos v.pos⟩

@[simp] instance : One DUnit where
  one := ⟨1, zero_lt_one⟩

@[simp] instance : MulOneClass DUnit where
  mul := (· * ·)
  one := 1
  mul_one u := by
    ext
    simp only [instMulDUnit]
    -- u.val * 1 = u.val
    exact mul_one u.val
  one_mul u := by
    ext
...
@[simp] lemma val_ne_zero (u : DUnit) : u.val ≠ 0 := ne_of_gt u.pos

/-- 換算係数： (u, x) を単位 w へ換算するときの「数値」倍率は x' = x * (u/w) -/
noncomputable def ratio (u w : DUnit) : ℝ := u.val / w.val

@[simp] lemma ratio_self (u : DUnit) : ratio u u = 1 := by
  unfold ratio; have := u.val_ne_zero; field_simp

@[simp] lemma ratio_comp (u v w : DUnit) :
...
/-- 異単位への換算： (u,x) ↦ (w, x * (u/w)) -/
noncomputable def convert (q : Qty) (w : DUnit) : Qty :=
  ⟨w, q.x * DUnit.ratio q.u w⟩

@[simp] lemma convert_id (q : Qty) : convert q q.u = q := by
  cases q with
  | mk u x =>
    ext <;> simp [convert, DUnit.ratio_self, mul_one]

@[simp] lemma convert_comp (q : Qty) (v w : DUnit) :
    convert (convert q v) w = convert q w := by
  cases q with
  | mk u x =>
    ext
    · rfl
    · have := DUnit.ratio_comp u v w
      simp [convert, this, mul_assoc]

/-- 同一単位ファイバー内の加法 -/
def addSame (u : DUnit) (a b : Qty) : Qty :=
  ⟨u, a.x + b.x⟩

/-- 異単位の加法：共通単位 w に揃えて足す -/
noncomputable def addVia (w : DUnit) (a b : Qty) : Qty :=
  let a' := convert a w
  let b' := convert b w
  ⟨w, a'.x + b'.x⟩

/-- 共通単位の選び方に依らず一意（換算の自然性） -/
@[simp] lemma addVia_natural (w₁ w₂ : DUnit) (a b : Qty) :
...
@[simp] lemma convert_natural_F (q : Qty) (w : DUnit) :
...
@[simp] lemma absVal_convert (q : Qty) (w : DUnit) :
    absVal (convert q w) = absVal q := by
  cases q with
  | mk u x =>
    -- (x * (u/w)) * w = x*u
    simp [absVal, convert, DUnit.ratio]
    field_simp

noncomputable def dimless (u0 : DUnit) (q : Qty) : ℝ :=
  (convert q u0).x

@[simp] lemma dimless_invariant (u0) (q : Qty) (w : DUnit) :
  dimless u0 (convert q w) = dimless u0 q := by
  cases q with
  | mk u x =>
    simp [dimless, Qty.convert, DUnit.ratio, mul_assoc]
...
def u1 : DUnit := ⟨1, by norm_num⟩
def u2 : DUnit := ⟨2, by norm_num⟩
noncomputable def wSqrt2 : DUnit :=
...
@[ext] structure QtyT (u : DUnit) where
  x : ℝ

namespace QtyT

instance (u : DUnit) : Zero (QtyT u) := ⟨⟨0⟩⟩
instance (u : DUnit) : Add  (QtyT u) := ⟨fun a b => ⟨a.x + b.x⟩⟩
instance (u : DUnit) : AddCommMonoid (QtyT u) where
  add_assoc a b c := by
    ext
    -- (a + b + c).x = (a.x + b.x + c.x)
    -- (a + (b + c)).x = (a.x + (b.x + c.x))
...
/-- 単位換算（線形） -/
noncomputable def convert {u w : DUnit} (q : QtyT u) : QtyT w :=
  ⟨ q.x * DUnit.ratio u w ⟩
...
@[simp] lemma convert_add {u w} (a b : QtyT u) :
  ...
  simp only [convert]
  -- ここで実数の分配則を使う
  exact add_mul a.x b.x (DUnit.ratio u w)
...
@[simp] lemma convert_zero {u w} :
  ...
  simp only [convert]
  -- 0 * anything = 0 を使う
  exact zero_mul (DUnit.ratio u w)
end QtyT
...
/-- log 座標（u > 0 なので安全） -/
noncomputable def logU (u : DUnit) : ℝ := Real.log u.val

@[simp] lemma logU_mapUnit (u : DUnit) :
...
instance (u : DUnit) : SMul ℝ (QtyT u) := ⟨fun r q => ⟨r * q.x⟩⟩

instance (u : DUnit) : AddCommMonoid (QtyT u) := inferInstance   -- ぬしの定義のままでOK

instance (u : DUnit) : Module ℝ (QtyT u) where
...
@[simp] lemma convert_smul {u w} (r : ℝ) (q : QtyT u) :
  ...
  ext
  simp only [convert]
  change (r * q.x) * DUnit.ratio u w = r * (q.x * DUnit.ratio u w)
  rw [mul_assoc]

/-- 単位換算は線形同型（可逆） -/
noncomputable def convertLinEquiv {u w : DUnit} : QtyT u ≃ₗ[ℝ] QtyT w :=
{ toLinearMap :=
  { toFun := fun q => convert q,
    map_add' := by
      ...
      ext
      simp only [convert]
      change (a.x + b.x) * DUnit.ratio u w = a.x * DUnit.ratio u w + b.x * DUnit.ratio u w
      exact add_mul a.x b.x (DUnit.ratio u w),
    map_smul' := by
      intro r q
      ext
...
@[simp] lemma addVia_comm (w : DUnit) (a b : Qty) :
    addVia w a b = addVia w b a := by
  unfold addVia; simp [convert, add_comm]

@[simp] lemma addVia_assoc (w : DUnit) (a b c : Qty) :
...
/-- 単位の n 乗 -/
def mapUnitN (n : ℕ) (u : DUnit) : DUnit :=
...
@[simp] lemma convert_natural_FN (n : ℕ) (q : Qty) (w : DUnit) :
  convert (liftFN n q) (mapUnitN (n + 1) w) = liftFN n (convert q w) := by
  cases q with
  | mk u x =>
    ext
    · rfl
    · simp [liftFN, mapUnitN, convert, DUnit.ratio, pow_succ]
      field_simp [u.val_ne_zero, w.val_ne_zero]
...
@[simp] lemma ratio_mapUnitN (n : ℕ) (u w : DUnit) :
...
@[simp] lemma ratio_mul_symm (u w : DUnit) :
...
@[simp] lemma ratio_symm_mul (u w : DUnit) :
