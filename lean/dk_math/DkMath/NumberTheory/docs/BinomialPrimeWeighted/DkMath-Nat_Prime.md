...
リーマンゼータ関数よりも、素直なのでどの素数の寄与が、というのは読みやすいですが。無いよりマシです。観測ツールは多いほうが良い。
連続化したときに、ダイヤルが滑らかな回転を伴わなければならないと思うので、瞬時に回るダイヤルも、何回転して揃ったのか見えるかもしれません。スローモーション。
ｎ行までは、それ以降のｎ＋１行で現れる大きな素数は絶対に現れないので、組合せ論的な解釈もできますね。
中間行の無限和級数状態でも、でてこないと思われるが、見たこと無いので解らないな。そもそも整数構成で無限だと発散してしまうような。収束する実数な係数なのですかね？
とりあえず、そんな係数でも、因子構造があってダイヤルが中途半端な回転状態でも正回転しているという状況です。そうであって欲しい。

はい、この観測はゼータ関数よりかなり「局所的に読める」利点があります。`C(n,k)` は有限個の整数なので、
...
が `padicValNat p (Nat.choose n k)` でそのまま見える。これは強いです。

組合せ論的にも重要なのはここですね。

```text
row n の係数 C(n,k) の素因子は、必ず n 以下
```

なぜなら `C(n,k)` は `n!` 由来なので、`n` より大きい素数は出現できない。したがって、

```text
n 行までは、n+1 以降で初めて現れる大きな素数は絶対に見えない
```

これは「素数ダイヤルがまだ存在しない / まだ回り始めていない」と読めます。Lean でも次にかなり自然に固定できます。
...
これは既に `below_prime_not_dvd_inner_choose` に近いですが、内側係数に限らず全係数へ広げると、ダイヤル観測として使いやすいです。

連続化については、注意点があります。整数係数そのものを無限行へ進めると、普通は発散します。なので連続化・級数化するなら、たぶん正規化が必要です。
...
これは二項分布です。総和が常に `1` になるので、無限・極限を考えやすい。

```text
Σ_k μ_n(k) = 1
```

このとき係数は実数になりますが、因子構造は分子側の `C(n,k)` に残ります。つまり、
...
「中途半端な回転」は、Lean 的にはすぐ p-adic valuation だけで表すより、
...
のペアとして扱うのがよいです。p-adic 高さは離散的、正規化質量は連続化可能。この2つを合わせると、スローモーション観測に近づきます。

次の小さな実装候補はこれです。

```lean
/-- No prime larger than the row index can appear in a Pascal coefficient. -/
theorem prime_not_dvd_pascalCoeffMass_of_row_lt
    {p n k : ℕ} (hp : p.Prime) (hnp : n < p) :
    ¬ p ∣ pascalCoeffMass n k
```

これを `PascalPrimeDial` に置けば、

```text
未到達の素数ダイヤルは row n では回らない
```
