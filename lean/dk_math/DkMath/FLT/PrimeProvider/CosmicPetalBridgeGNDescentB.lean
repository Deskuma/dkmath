...
/--
canonical eq-side trace core から `Nums` 部分だけを回収する。

eq-side trace の「数値生成」切断面。
-/
...
/-- canonical eq-side trace core から `Eq` witness を回収する。 -/
...
/-- canonical eq-side trace core から `Inv` witness を回収する。 -/
