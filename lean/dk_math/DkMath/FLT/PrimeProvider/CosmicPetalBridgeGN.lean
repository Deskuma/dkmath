...
/-- Branch B 条件付きで、`z` 最小の反例 pack を no-`sorry` で選べる。 -/
...
/-- Branch B の `反例 => lift` が供給されれば、最小反例 + lift の選択仕様は no-`sorry` で得られる。 -/
