...
これらは全て既存 default 補題で so#rry なしに構成可能。
-/
...
/--
**`v_q(N % q^k) = v_q(N)` for `v_q(N) < k`** (so#rry-free 補題).
...
/-- `GN` の ZMod (q^k) への cast の val = GN % q^k (so#rry-free). -/
...
/--
**GN の ZMod (q^k) での valuation = p * v_q(s)** (key so#rry).

`v_q(N % q^k) = v_q(N)` (when v_q(N) < k) と `GN = p * s^p` から導く。

so#rry の理由: `branchA_padicValNat_mod_pow_eq` が未完成（片側に so#rry あり）。
-/
