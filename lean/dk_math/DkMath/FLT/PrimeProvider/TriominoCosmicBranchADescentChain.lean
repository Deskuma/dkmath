...
後段の `2m-geom/core`（`QAdicGapReduction` の最小核）に圧縮できる。
...
`2m-int` / `2m-geom` / `2m-core` へ圧縮可能であり、
実際の主戦場は `2m-geom/core` にある。
-/
...
Level 2m-core: QAdicGapReductionCore (pack + strong witness ⇒ g') — final 1-kernel candidate ★OPEN★
```
...
さらに bookkeeping を剥いだ `Level 2m-core` が
最終 1 核候補として自然に現れる。
