...
/-! A neutral finite-field criterion for the real cubic with conductor seven. -/
...
theorem cubicRoot_mod_seven
  ...
  · left
    have hq1 : (q : ZMod 7) = 1 := sub_eq_zero.mp h
    exact (ZMod.natCast_eq_natCast_iff' q 1 7).mp hq1 |>.trans (by norm_num)
  · right
    have hqneg : (q : ZMod 7) = -1 := eq_neg_of_add_eq_zero_left h
    have hq6 : (q : ZMod 7) = (6 : ZMod 7) := by
      calc
        (q : ZMod 7) = -1 := hqneg
        _ = (6 : ZMod 7) := by
          have h7 : (7 : ZMod 7) = 0 := ZMod.natCast_self 7
          calc
            ...
            _ = 6 := by rw [h7]; simp
    exact (ZMod.natCast_eq_natCast_iff' q 6 7).mp hq6 |>.trans (by norm_num)
