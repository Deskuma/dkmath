...
Mathlib の `ClassGroup.mk_eq_one_of_coe_ideal` 系 API が候補になる領域。
-/
...
現時点では最後の extraction target を返すだけの abstract composition。
-/
...
現時点では placeholder target なので clean に通す。
将来は `ClassGroup.mk_eq_one_of_coe_ideal` などの API を使う段になる。
-/
