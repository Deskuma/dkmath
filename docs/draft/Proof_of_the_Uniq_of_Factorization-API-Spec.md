...
## 10. 記号定義の一貫性チェック（`Exc` / `NonExc`）

本節では、Lean 名・論文記法・条件記述が同じ層意味を指しているかを確認する。

### 10.1 ルール

1. `Exc` は必ず「例外層」= 条件 `q ∣ d` を表す。
2. `NonExc` は必ず「非例外層」= 条件 `q ∤ d` を表す。
3. `\neg\mathrm{Exc}` 記法を使う対象は、Lean 名でも `nonExceptional*` 系に対応させる。
4. `NonExceptionalBridgeEntrance` / `NonExceptionalBoundaryEntrance` は
   いずれも非例外層専用 facade とする（例外層用途に流用しない）。
5. `autoExc*` は「例外層側の自動供給を含む」意味で固定し、
   非例外層まで自動化する場合は `autoExcNonExc*` を使う。

### 10.2 現在のチェック結果

| チェック項目 | 判定 | 補足 |
|---|---|---|
| `Exc` 記法と `q ∣ d` 条件の整合 | OK | 対応表・本文とも一致 |
| `NonExc` 記法と `q ∤ d` 条件の整合 | OK | 対応表・本文とも一致 |
| `\neg\mathrm{Exc}` と `nonExceptional*` 命名の整合 | OK | bridge/boundary facade で統一 |
| `autoExcMK` / `autoExcNonExcMK` の意味分離 | OK | API 名と説明が一致 |
| 推奨入口の層記法（final）整合 | OK | `nonExcFacade` + `autoExcNonExcMK` で明示 |
